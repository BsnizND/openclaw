...
describe("subagent announce seam flow", () => {
  ...
  it("retains a delete-mode child until retryable completion delivery settles", async () => {
    ...
    await expect(runSubagentAnnounceFlow(params)).resolves.toBe("retryable");
    expect(sessionsDeleteSpy).not.toHaveBeenCalled();

    await expect(runSubagentAnnounceFlow(params)).resolves.toBe("delivered");
    expect(sessionsDeleteSpy).toHaveBeenCalledTimes(1);
    expect(sessionsDeleteSpy).toHaveBeenCalledWith({
      method: "sessions.delete",
      params: {
  ...
  it("still deletes after intentional completion non-delivery", async () => {
    ...
    const announceOutcome = await runSubagentAnnounceFlow({
      childSessionKey: "agent:main:subagent:retired",
    ...
    expect(announceOutcome).toBe("intentional_non_delivery");
    expect(agentSpy).not.toHaveBeenCalled();
    expect(sessionsDeleteSpy).toHaveBeenCalledTimes(1);
  });
