...
| `allowQueueControls`  | `false`                 | Allow owner-authorized `codex_threads` queue writes. This separate grant does not enable the existing native or compatibility mutations.                                                                                                  |
| `queueEndpoint`       | unset                   | Trusted absolute local `unix://` socket URL used only by `codex_threads` queue and queue-list requests. Both actions fail closed when this endpoint is absent or invalid.                                                                 |
