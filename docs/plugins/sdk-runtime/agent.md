...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    `readPreferredUpstreamUserText(message)` preserves the native distinction between upstream user content and internal mirrored input: `null` excludes mirror-only input, an empty string is valid empty upstream text (including media-only turns), and `undefined` means no upstream text override. When choosing the most recent user interaction from visible entries, retain user messages with empty text and exclude only the canonical mirror-only result.

    For native channel delivery that must persist before notifying a client, the same runtime subpath exports `appendAssistantMessageToSessionTranscript(...)`. Pass the exact `sessionKey` and `expectedSessionId`, `config`, text and/or canonical `media` facts, and a stable per-send `idempotencyKey` when available. For independently identified text sends, also set `deliveryMirror: { kind: "channel-final", sourceMessageId: deliveryId }` so identical text from different send IDs remains separate while retries retain the original entry. This is the canonical assistant writer: managed image retention, writer/lifecycle fences, idempotency conflict detection, and transcript update publication remain owned by OpenClaw. Check `ok` before notifying and retain the returned `messageId`; a failed append is not a successful delivery. Do not create another transcript writer or infer a committed message from an outbound operation ID.
